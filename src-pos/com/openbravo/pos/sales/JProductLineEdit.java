...
public class JProductLineEdit extends javax.swing.JDialog {
    ...
    private boolean m_bdiscounts;
    
    private SentenceList m_sentdiscount;
    private ComboBoxValModel m_DiscountModel;
    
            
    /** Creates new form JProductLineEdit */
    private JProductLineEdit(java.awt.Frame parent, boolean modal, boolean showDiscounts) {
        this(parent, modal);
        m_bdiscounts = showDiscounts;
    }
    ...
    /** Creates new form JProductLineEdit */
    private JProductLineEdit(java.awt.Dialog parent, boolean modal, boolean showDiscounts) {
        this(parent, modal);
        m_bdiscounts = showDiscounts;
    }
    ...
    private boolean init(AppView app, TicketLineInfo oLine) {
        ...
        if (m_oLineTicket.isDiscount() || m_oLineTicket.isSubproduct() || !m_bdiscounts) {
            m_jDiscount.setVisible(false);
            m_jLblDiscount.setVisible(false);
        } else {
            try {
                // Rellenamos el ComboBox con los descuentos
                DataLogicSales dlSales = (DataLogicSales) app.getBean("com.openbravo.pos.forms.DataLogicSalesCreate");
                m_sentdiscount = dlSales.getDiscountList();
                m_DiscountModel = new ComboBoxValModel(m_sentdiscount.list());
                m_DiscountModel.add(null);
                m_jDiscount.setModel(m_DiscountModel);
                m_jDiscount.setSelectedItem(m_oLineTicket.getDiscountInfo());
    ...
    public static boolean showMessage(Component parent, AppView app, TicketLineInfo oLine, boolean showDiscounts) {
        Window window = getWindow(parent);
        
        JProductLineEdit myMsg;
        if (window instanceof Frame) { 
            myMsg = new JProductLineEdit((Frame) window, true, showDiscounts);
        } else {
            myMsg = new JProductLineEdit((Dialog) window, true, showDiscounts);
        }
        return myMsg.init(app, oLine);
    }   
    ...
    private void initComponents() {
        ...
        m_jDiscount = new javax.swing.JComboBox();
        ...
        m_jDiscount.setBounds(100, 230, 210, 30);
    ...
    private void m_jButtonOKActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jButtonOKActionPerformed
        ...
        try {
            m_oLineTicket.setDiscountInfo( (DiscountInfo)m_DiscountModel.getSelectedItem() );
        } catch (Exception e) {
            m_oLineTicket.setDiscountInfo(null);
        }
    ...
    private javax.swing.JComboBox m_jDiscount;
